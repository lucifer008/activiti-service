 ...
 */
@Slf4j
public class FinanceTaskListener implements TaskListener, JavaDelegate {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1883816508974843966L;
	...
	public void notify(DelegateTask delegateTask) {
		...
		// log.info("AuditTaskListener",message);

		// 组任务
		String taskId = delegateTask.getId();
		List<SubApprovalModel> nextAuditUserList = (List<SubApprovalModel>) delegateTask.getVariable("auditUsers");
		...
		for (SubApprovalModel users : nextAuditUserList) {
			...
			log.info(
					"【ApprovalTaskListener-->notify 】下级审核待办人任务Id" + delegateTask.getId() + " 待办人:" + users.getUserId());
			index++;
		}
		// delegateTask.addCandidateGroup(""+RoleType.Forensic.getRoleType());

		// log.info("【ApprovalTaskListener-->notify 】下级审核待办人任务Id"+delegateTask.getId());
	}
