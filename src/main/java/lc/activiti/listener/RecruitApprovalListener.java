...
@Slf4j
public class RecruitApprovalListener implements TaskListener,JavaDelegate {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        if (nextApprovalList == null || nextApprovalList.size() == 0) {
            String info="【招聘审批ApprovalListener-->notify 】下级审批人为空!任务Id-->" + delegateTask.getId();
            log.info(info);
        ...
        for (SubApprovalModel users : nextApprovalList) {
            ...
            log.info("【招聘审批 ApprovalListener-->notify 】下级审批人任务Id-->" + delegateTask.getId() + " 待办人:" + users.getUserId());
            index++;
        }
    }
}
